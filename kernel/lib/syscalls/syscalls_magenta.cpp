...
mx_handle_t sys_thread_create(mx_handle_t process_handle, const char* name, uint32_t name_len, uint32_t flags) {
    LTRACEF("flags 0x%x\n", flags);
...
mx_handle_t sys_process_create(const char* name, uint32_t name_len, uint32_t flags) {
...
mx_status_t sys_process_start(mx_handle_t process_handle, mx_handle_t thread_handle, uintptr_t entry, uintptr_t stack, mx_handle_t arg_handle_value) {
    ...
    // XXX test thread_rights

    // XXX test that thread belongs to process

    // XXX test that handle has TRANSFER rights
